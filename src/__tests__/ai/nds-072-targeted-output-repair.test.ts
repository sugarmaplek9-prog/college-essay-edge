...
import {
  collectBestDirectionText as collectV2BestDirectionText,
  computeSourceOverlap as computeV2SourceOverlap,
  run072Case,
...
function resolveFrozenInputPath(): string {
  const candidates = [
    path.join(
      process.cwd(),
      'evaluation_outputs',
...
const INPUT_PATH = resolveFrozenInputPath();
...
describe('072 targeted output repair — V2 guardrails', () => {
  ...
  it('keeps representative 072 outputs grounded in case-specific source language', async () => {
    for (const caseId of ['RHC-001', 'RHC-026', 'RHC-030']) {
      ...
      const outputText = collectV2BestDirectionText(payload);
      const overlap = computeV2SourceOverlap(caseRecord, outputText);
