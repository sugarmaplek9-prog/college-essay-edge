...
replacement_before_escape = """const RECOMMENDATION_DIRECTIVE_STARTERS = [
...
"""
text = text[:start] + replacement_before_escape + text[end:]

escape_start = text.find("function escapeRegex(value: string): string {")
strip_start = text.find("function stripDirectivePrefix(directionLine: string): string {")
if escape_start < 0 or strip_start < 0 or strip_start <= escape_start:
    raise SystemExit('Failed to locate escape/strip blocks')

replacement_escape_to_strip = """function escapeRegex(value: string): string {
...
text = text[:escape_start] + replacement_escape_to_strip + text[strip_start:]
