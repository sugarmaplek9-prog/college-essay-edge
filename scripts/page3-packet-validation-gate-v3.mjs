...
const REQUIRE_BASELINE_COMPLETE = process.env.REQUIRE_BASELINE_COMPLETE !== 'false';
const REQUIRE_SCORED_CASE_COUNT = process.env.REQUIRE_SCORED_CASE_COUNT !== 'false';
...
function maybeReadJson(p, fallback) {
  if (!fs.existsSync(p)) return fallback;
  return JSON.parse(fs.readFileSync(p, 'utf-8'));
}

function main() {
  const packet = maybeReadJson(PACKET_PATH, []);
  const provenance = maybeReadJson(PROVENANCE_PATH, null);
  const summary = maybeReadJson(SUMMARY_PATH, { rows: [] });

  const productRecs = [];
  const keyPath = path.join(OUT_DIR, 'blind_review_answer_key.json');
  const key = fs.existsSync(keyPath) ? (JSON.parse(fs.readFileSync(keyPath, 'utf-8')).key ?? []) : [];
  const keyByCase = new Map(key.map((row) => [row.case_id, row]));

  for (const row of packet) {
    const k = keyByCase.get(row.case_id);
    if (!k) continue;
  ...
  if (productRecs.length === 0) {
    for (const row of summary.rows ?? []) {
      const rec = row?.product?.output?.displayed_recommendation;
      if (clean(rec)) productRecs.push(clean(rec));
    }
  }
  ...
  const baselineComplete = provenance?.packet_baseline_complete ?? provenance?.baseline_complete ?? summary?.packet_baseline_complete ?? false;
  const scoredCaseCount = Number(provenance?.packet_scored_case_count ?? provenance?.scored_case_count ?? summary?.packet_scored_case_count ?? 0);

  const gates = {
    provenance_present: Boolean(provenance),
    current_run_only_packet: String(provenance?.packet_source ?? '').startsWith('current_run_'),
    baseline_complete: REQUIRE_BASELINE_COMPLETE ? baselineComplete : true,
    scored_case_count_positive: REQUIRE_SCORED_CASE_COUNT ? scoredCaseCount > 0 : true,
  ...
  const out = {
    ...
    thresholds: {
      ...
      require_scored_case_count: REQUIRE_SCORED_CASE_COUNT,
    },
  ...
  const md = [];
  md.push('# PAGE3 PACKET VALIDATION GATE V3');
  md.push('');
