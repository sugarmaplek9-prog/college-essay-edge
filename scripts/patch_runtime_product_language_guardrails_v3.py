...
canonical_path = root / 'src/lib/fm/canonicalPage3Payload.ts'

direction = direction_path.read_text()
canonical = canonical_path.read_text()

direction = replace_strict(direction, """function round(value: number): number {
  return Number(value.toFixed(3));
}

function buildRuntimeCandidates(""", """function round(value: number): number {
  return Number(value.toFixed(3));
}
...
function buildRuntimeCandidates(""", 'insert helper guardrails')

direction = replace_strict(direction, """      why_this_direction: normalizeSentence(why_this_direction),
      essay_about: normalizeSentence(essay_about),
      next_move: normalizeSentence(next_move),""", """      why_this_direction: ensureComparativeWhy(why_this_direction),
      essay_about: normalizeSentence(essay_about),
      next_move: ensureConcreteNextMove(next_move),""", 'recommend normalization wiring')

direction = replace_strict(direction, """    const lowSpecificityRisk = uniqueAnchorCount < 2 || duplicateAnchorRatio > 0.34;

    const quotedFragments = Array.from(""", """    const lowSpecificityRisk = uniqueAnchorCount < 2 || duplicateAnchorRatio > 0.34;
    ...
    const quotedFragments = Array.from(""", 'score signals injection')

direction = replace_strict(direction, """    const aboutOverlap = jaccardOverlap(candidate.direction_line, candidate.essay_about);
    const essayAboutRedundancyPenalty = clampScore(""", """    const aboutOverlap = jaccardOverlap(candidate.direction_line, candidate.essay_about);
    ...
    const essayAboutRedundancyPenalty = clampScore(""", 'essay_about meaning signal')

direction = replace_strict(direction, """    const identifiabilityPenalty = clampScore((rhythmTellHits >= 3 ? 0.3 : 0.1) + (bannedComparePattern.test(text) ? 0.4 : 0));

    const prePenaltyTotal = round(""", """    const identifiabilityPenalty = clampScore((rhythmTellHits >= 3 ? 0.3 : 0.1) + (bannedComparePattern.test(text) ? 0.4 : 0));
    ...
    const prePenaltyTotal = round(""", 'human clarity score insertion')

direction = replace_strict(direction, """      caseSpecificityBeyondPivot * 0.08 +
...
direction = replace_strict(direction, """      contradictionOverusePenalty * 0.16 -
...
direction = replace_strict(direction, """    if (essayAboutRedundancyPenalty > 0.25) rejectionReasons.push('essay_about_redundancy_penalty');
    if (familyCollapsePenalty > 0.2) rejectionReasons.push('family_collapse_penalty');""", """    if (essayAboutRedundancyPenalty > 0.25) rejectionReasons.push('essay_about_redundancy_penalty');
    ...
    if (familyCollapsePenalty > 0.2) rejectionReasons.push('family_collapse_penalty');""", 'hard fail rejection reasons')

direction = replace_strict(direction, """  if (!needsDominanceCap) return diversityAdjusted;

  const winner = diversityAdjusted[0];""", """  const hardFailReasons = new Set([
  ...
  const winner = diversityAdjusted[0];""", 'hard fail winner gate')

direction = replace_strict(direction, """        weaker_read: renderGuard(
          [
          ...
          ][Math.abs(`${runnerUpCandidate.candidate_id || ''}:${runnerUpCandidate.hinge_span || ''}`.split('').reduce((n, ch) => n + ch.charCodeAt(0), 0)) % 3],""", """        weaker_read: renderGuard(
          [
          ...
          ][Math.abs(`${runnerUpCandidate.candidate_id || ''}:${runnerUpCandidate.hinge_span || ''}`.split('').reduce((n, ch) => n + ch.charCodeAt(0), 0)) % 3],""", 'weaker confusing framing')

direction = replace_strict(direction, """        stronger_read: renderGuard(
          [
          ...
          ][Math.abs(`${selectedCandidate?.candidate_id ?? 'selected'}:${selectedCandidate?.hinge_span ?? ''}`.split('').reduce((n, ch) => n + ch.charCodeAt(0), 0)) % 3],""", """        stronger_read: renderGuard(
          [
          ...
          ][Math.abs(`${selectedCandidate?.candidate_id ?? 'selected'}:${selectedCandidate?.hinge_span ?? ''}`.split('').reduce((n, ch) => n + ch.charCodeAt(0), 0)) % 3],""", 'stronger usable framing with next step')

canonical = replace_strict(canonical, """      first_coaching_step: content?.strongest.write_first_steps?.[0] ?? ambiguityFallback?.first_coaching_step ?? null,""", """      first_coaching_step: content?.strongest.next_move ?? content?.strongest.write_first_steps?.[0] ?? ambiguityFallback?.first_coaching_step ?? null,""", 'first coaching step from strongest next move')
