...
const DIRECTION_PATH = process.env.DIRECTION_PATH
...
const OUT_JSON = path.join(OUT_DIR, 'WHY_CONTRACT_ALIGNMENT_AUDIT_V2.json');
const OUT_MD = path.join(OUT_DIR, 'WHY_CONTRACT_ALIGNMENT_AUDIT_V2.md');
...
const report = {
  ...
  comparative_leads: comparativeLeads,
...
const md = [
  '# Why Contract Alignment Audit V2',
  '',
...
console.log(JSON.stringify({
  out_json: OUT_JSON,
  out_md: OUT_MD,
  contract_alignment_ok: report.counts.contract_alignment_ok,
  comparative_failures: report.counts.comparative_failures,
  payoff_failures: report.counts.payoff_failures,
}, null, 2));
