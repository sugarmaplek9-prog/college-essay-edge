...
const BASELINE_SUMMARY = process.env.BASELINE_SUMMARY ?? path.join(ROOT, 'evaluation_outputs', 'page3_holdout_v2', 'summary.json');
...
function pickStarter(caseId) {
  ...
  let n = 0;
  for (const ch of String(caseId)) n += ch.charCodeAt(0);
  return starters[n % starters.length];
}
...
function liftEssayAbout(rec, about, why) {
  ...
  return `This essay is about the ${anchor} you adopted and how that changed your behavior in ways the reader can verify.`;
}

function productizeCompare(text, mode) {
  const t = clean(text);
  if (!t) {
    return mode === 'weaker'
      ? 'The weaker version stays at the event but does not reveal what changed in your approach.'
      : 'The stronger version reveals what changed in your standard and why that gives the essay a clearer reason to care.';
  }
  if (/\b(governing claim|hinge|selected option|alternate option|consequence cleanly)\b/i.test(t)) {
    return mode === 'weaker'
      ? 'The weaker version has detail, but it stays at what happened instead of naming what changed in your approach.'
      : 'The stronger version names what changed in your standard and shows why that makes the essay more compelling.';
  }
  return t;
}
...
function main() {
  ...
  for (const row of current.rows ?? []) {
    ...
    const pAbout = liftEssayAbout(pRec, pOut.essay_about, pOut.why_this_direction);

    const productPayload = {
      recommendation: pRec,
      essay_about: pAbout,
      why_this_direction: clean(pOut.why_this_direction),
      weaker_read: productizeCompare(pOut.weaker_read, 'weaker'),
      stronger_read: productizeCompare(pOut.stronger_read, 'stronger'),
  ...
  const provenance = {
    ...
    case_count: (current.rows ?? []).length,
