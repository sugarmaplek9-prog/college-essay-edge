...
AFTER = "evaluation_outputs/page3_controlled_testing_v1/ct24_coach_quality_v2/CONTROLLED_TESTING_SUMMARY_V1.json"
...
def summarize(path):
    ...
    top_prefix = bs["top_prefixes"][0]
    return {
        "relationship_winner_share_all": round(families.get("relationship", 0) / max(n, 1), 4),
        "top_shell": {"key": top_shell["key"], "count": top_shell["count"], "ratio": round(top_shell["count"] / n, 4)},
        "top_prefix": {"key": top_prefix["key"], "count": top_prefix["count"], "ratio": round(top_prefix["count"] / n, 4)},
