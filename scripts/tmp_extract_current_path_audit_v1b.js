...
function renderedExcerpt(output) {
  if (!output || typeof output !== 'object') return '';
  const candidate = output.body_excerpt || output.displayed_recommendation || '';
  return String(candidate).replace(/\s+/g, ' ').trim().slice(0, 1000);
}
...
function toCase(caseId) {
  ...
  return {
    ...
    rendered_output_excerpt: renderedExcerpt(r.product?.output),
  };
}
