...
BEFORE = "evaluation_outputs/page3_controlled_testing_v1/ct24_winner_path_v2/CONTROLLED_TESTING_SUMMARY_V1.json"
AFTER = "evaluation_outputs/page3_controlled_testing_v1/ct24_source_patch_v1/CONTROLLED_TESTING_SUMMARY_V1.json"
...
def summarize(path):
    ...
    n = len(rows)
    direction_rows = [r for r,_ in reviewed if r.get("route_category") == "direction"]
    direction_n = len(direction_rows)
    return {
        "case_count": n,
        "direction_count": direction_n,
        "relationship_winner_share_all": round(families.get("relationship", 0) / max(n, 1), 4),
        "relationship_winner_share_direction": round(families.get("relationship", 0) / max(direction_n, 1), 4),
        ...
        "score_means": {
            "recommendation_clarity": round(sum(f["recommendation_clarity"] for _,f in reviewed)/n, 4),
            "essay_about_meaning": round(sum(f["essay_about_meaning"] for _,f in reviewed)/n, 4),
