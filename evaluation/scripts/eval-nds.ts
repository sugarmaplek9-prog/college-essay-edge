...
async function main(): Promise<void> {
  ...
  const ndsResults = await runNdsEvaluation(selected);

  const outPath = path.join(runDir, 'nds_results.json');
  fs.writeFileSync(outPath, JSON.stringify(ndsResults, null, 2), 'utf-8');

  const perCaseDir = path.join(runDir, 'nds_cases');
  fs.mkdirSync(perCaseDir, { recursive: true });
  for (const row of ndsResults) {
    fs.writeFileSync(path.join(perCaseDir, `${row.case_id}.json`), JSON.stringify(row, null, 2), 'utf-8');
  }

  console.log(`NDS evaluation complete. runDir=${runDir} cases=${selected.length}`);
}
